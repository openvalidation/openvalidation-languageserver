...
export class AliasHelper {
  ...
  public getKeywordByAliasKey(keywordToFind: AliasKey): string | null {
    const foundKeyword = this.getKeys().find(
      key => this.aliases.get(key) === keywordToFind
    );
    if (!foundKeyword) {
      return null;
    }
  ...
  public getKeywordsByAliasKey(keywordToFind: AliasKey): string[] {
    return this.getKeys().filter(
      key => this.aliases.get(key)!.indexOf(keywordToFind) !== -1
    );
  }
  ...
  public getOperators(startingWord?: string): Map<string, [string, string]> {
    ...
    this.aliases.forEach((value: string, operatorKey: string) => {
      if (
        value.indexOf(AliasKey.OPERATOR) !== -1 &&
    ...
    const returnMap: Map<string, [string, string]> = new Map<
      string,
      [string, string]
    >();
