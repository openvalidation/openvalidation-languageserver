...
export class OvServer {
     ...
     * @memberof OvServer
     */
    public async setLanguage(params: { language: string, uri: string }): Promise<void> {
        if (!params) { return; }
     ...
     * @memberof OvServer
     */
    public async setCulture(params: { culture: string, uri: string }): Promise<void> {
        if (!params) { return; }
    ...
    public async setAliases(): Promise<void> {
        ...
        if (!!commentKeyword) {
            this.connection.sendNotification(NotificationEnum.CommentKeywordChanged, commentKeyword);
        }
    }
