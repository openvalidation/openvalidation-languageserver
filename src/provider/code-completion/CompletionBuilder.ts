...
export class CompletionBuilder {
    ...
    public addFittingChilds(parentName: string): CompletionBuilder {
        this.schema.complexData.forEach(property => {
            ...
            if (property.parent === manipulatedParent) {
                const schemaProperty: ISchemaProperty[] =
                    this.schema.dataProperties.filter(p => p.name === property.child);
                const dataType: string | null = schemaProperty.length > 0 ? schemaProperty[0].type : null;
                if (!!dataType) {
    ...
    public addOperandsWithTypeOfGivenOperand(operandName: string): CompletionBuilder {
        const variables: Variable | undefined =
            this.declarations.find(declaration => declaration.$name === operandName);
        if (!!variables) {
            return this.addFittingIdentifier(new OperandTransition(variables.$dataType, [operandName], ' '));
        }

        const schema: ISchemaProperty | undefined =
            this.schema.dataProperties.find(property => property.name === operandName);
        if (!!schema) {
    ...
    private addVariable(
        ...
        const completionItem = this.createCompletionItemWithTextInsertion(label, label, sortText, prependedText);
    ...
    private addSnippet(label: string | null, text: string, sortText: string, prependedText: string): CompletionBuilder {
        ...
        const completionItem = this.createCompletionItemWithTextInsertion(label, text, sortText, prependedText);
