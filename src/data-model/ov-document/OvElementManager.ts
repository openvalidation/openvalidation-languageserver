...
export class OvElementManager {
  ...
  public getVariables(): VariableNode[] {
    return this.elements.filter(
      element => element instanceof VariableNode
    ) as VariableNode[];
  }
  ...
  public getComments(): CommentNode[] {
    return this.elements.filter(
      element => element instanceof CommentNode
    ) as CommentNode[];
  }
  ...
  public getElementsByRange(range: Range): GenericNode[] {
    const elements = this.elements.filter(rule => {
      ...
      if (!elementRange || !elementRange.$end || !elementRange.$start) {
        return false;
      }
  ...
  public getVariablesByName(name: string): VariableNode[] | null {
    const filteredVariables: VariableNode[] = this.getVariables().filter(
      element =>
        !!element.$nameNode &&
        element.$nameNode.$name.toLowerCase() === name.toLowerCase()
    );
  ...
  public getUsedVariables(
    ...
    for (const variable of this.getVariables()) {
      ...
      const asKeywordNameString =
        asKeyword.toLowerCase() + " " + variable.$nameNode.$name.toLowerCase();
