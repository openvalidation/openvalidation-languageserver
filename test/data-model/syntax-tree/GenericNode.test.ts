...
describe('GenericNode Tests', () => {
    ...
    test('formatCode with good formatted OperandNode, expect no edit', () => {
        const node: GenericNode = new OperandNode(['Test'], IndexRange.create(0, 0, 0, 4), 'String', 'Test');

        const expected: string = 'Test';
        const actual: string = node.formatCode(initializer.$server.aliasHelper)[0].newText;
