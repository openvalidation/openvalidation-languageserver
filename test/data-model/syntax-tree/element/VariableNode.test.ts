...
describe('VariableNode Tests', () => {
    ...
    test('VariableNode get/set value/nameNode test', () => {
        const variableNode: VariableNode =
            new VariableNode(null, null, ['Wenn Alter gleich 18  Dann '], IndexRange.create(0, 0, 0, 27));
    ...
    test('getChildren without child, expect no children', () => {
        const variable: VariableNode =
            new VariableNode(null, null, [' Als Test'], IndexRange.create(0, 0, 0, ' Als Test'.length));
    ...
    test('getChildren with one child, expect one child', () => {
        ...
        const leftOperand: OperandNode = new OperandNode(['Alter'], IndexRange.create(0, 0, 0, 5), 'Decimal', 'Alter');
        const operator: OperatorNode = new OperatorNode(['gleich'], IndexRange.create(0, 6, 0, 12), 'Boolean', 'EQUALS', 'Object');
        const rightOperand: OperandNode =
            new OperandNode(['18'], IndexRange.create(0, 13, 0, 15), 'Decimal', '18');
        const operation =
            new OperationNode(leftOperand, operator, rightOperand, ['Alter gleich 18'], IndexRange.create(0, 0, 0, 15));

        const variable: VariableNode =
            new VariableNode(variableNameNode, operation, ['Alter gleich 18  Als Test'], IndexRange.create(0, 0, 0, 'Alter  Als Test'.length));
    ...
    test('getHoverContent without content, expect not empty content', () => {
        const variable: VariableNode =
            new VariableNode(null, null, ['Alter  Als Test'], IndexRange.create(0, 0, 0, 'Alter  Als Test'.length));
    ...
    test('getBeautifiedContent without children, expect not empty content', () => {
        const variable: VariableNode =
            new VariableNode(null, null, [' As Test'], IndexRange.create(0, 0, 0, 'Alter  Als Test'.length));

        const actual = variable.getBeautifiedContent(initializer.$server.aliasHelper);
    ...
    test('getBeautifiedContent with children, expect not empty content', () => {
        ...
        const leftOperand: OperandNode = new OperandNode(['Alter'], IndexRange.create(0, 0, 0, 5), 'Decimal', 'Alter');
        const variable: VariableNode =
            new VariableNode(variableNameNode, leftOperand, ['Alter  As Test'], IndexRange.create(0, 0, 0, 'Alter  Als Test'.length));
