...
describe('CommentNode Tests', () => {
    ...
    test('getCompletionContainer with empty CommentNode, expected Empty', () => {
        const comment: CommentNode = new CommentNode([], IndexRange.create(0, 0, 0, 0), '');

        const positionParameter = Position.create(0, 5);

        const actual: StateTransition[] = comment.getCompletionContainer(positionParameter).$transitions;
        expect(actual[0]).toBeInstanceOf(EmptyTransition);
    });

    test('getCompletionContainer with full CommentNode, expected Empty', () => {
        const comment: CommentNode = new CommentNode(['Kommentar Das ist ein Test'], IndexRange.create(0, 0, 0, 'Kommentar Das ist ein Test'.length), 'Das ist ein Test');
    ...
    test('getChildren test, expect no children', () => {
        ...
        const commentNode: CommentNode =
            new CommentNode([commentText], IndexRange.create(0, 0, 0, commentText.length), commentText);
    ...
    test('getBeautifiedContent test with aliasHelper and one line, expect not empty content', () => {
        ...
        const actual = commentNode.getBeautifiedContent(initializer.$server.aliasHelper);
    ...
    test('getBeautifiedContent test with aliasHelper and two lines, expect not empty content', () => {
        const commentText: string = 'This is an error';
        const commentNode: CommentNode =
            new CommentNode([commentText, commentText], IndexRange.create(0, 0, 1, commentText.length), commentText);
    ...
    test('getCompletionContainer test, expect not empty content', () => {
        ...
        const actual: StateTransition = commentNode.getCompletionContainer(Position.create(0, 5)).$transitions[0];
