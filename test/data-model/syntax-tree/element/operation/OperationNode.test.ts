...
describe("Operation Tests", () => {
  ...
  test("getCompletionContainer with OperationNode with staticOperand, expected correct Datatype", () => {
    ...
    const actual: string | undefined = (operation.getCompletionContainer(
      positionParameter
    ).$transitions[0] as OperatorTransition)!.$dataType;

    expect(actual).toEqual(expected);
  });

  test("getCompletionContainer with OperationNode with staticArrayOperand, expected Operands", () => {
    const item: OperandNode = new OperandNode(
      ["Alter"],
      IndexRange.create(0, 0, 0, 5),
      "Decimal",
      "Alter"
    );
    const leftOperand: ArrayOperandNode = new ArrayOperandNode(
      [item],
      ["Alter"],
      IndexRange.create(0, 0, 0, 5),
      "Decimal",
  ...
  test("getCompletionContainer with OperationNode with staticFunctionOperand, expected Operands", () => {
    ...
    const leftOperand: FunctionOperandNode = new FunctionOperandNode(
      [item],
      ["Alter"],
      IndexRange.create(0, 0, 0, 5),
      "Decimal",
  ...
  test("getCompletionContainer with OperationNode with staticFunctionOperand, expected correct DataType", () => {
    ...
    const leftOperand: FunctionOperandNode = new FunctionOperandNode(
      ...
      IndexRange.create(0, 0, 0, "Summe von Einkaufsliste.Preis".length),
      "Decimal",
      "Alter",
      "Decimal"
    );
    ...
    const positionParameter = Position.create(
      0,
      "Summe von Einkaufsliste.Preis".length + 1
    );
  ...
  test("getCompletionContainer with OperationNode with Operand and Operation, expected correct OperatorDatatype", () => {
    ...
    const transition = operation.getCompletionContainer(positionParameter)
      .$transitions[0] as OperatorTransition;
  ...
  test("getCompletionContainer with OperationNode with staticFunctionOperand, expected FunctionOperand and ConnectedOperation", () => {
    const leftOperand: OperandNode = new OperandNode(
      ["18"],
      IndexRange.create(0, 0, 0, 2),
      "Decimal",
      "18.0"
    );
    const operator: OperatorNode = new OperatorNode(
      ["gleich"],
      IndexRange.create(0, 3, 0, 9),
      "Boolean",
      "EQUALS",
      "Object"
    );
    const item: OperandNode = new OperandNode(
      ["Einkaufsliste.Preis"],
      IndexRange.create(0, 20, 0, 39),
      "Decimal",
      "Einkaufsliste.Preis"
    );
    const rightOperand: FunctionOperandNode = new FunctionOperandNode(
      [item],
      ["Summe von Einkaufsliste.Preis"],
      IndexRange.create(0, 10, 0, 39),
      "Decimal",
      "Alter",
      "Decimal"
    );
  ...
  test("getCompletionContainer with OperationNode with staticArrayOperand, expected FunctionOperand and ConnectedOperation", () => {
    ...
    const item: OperandNode = new OperandNode(
      ["Alter"],
      IndexRange.create(0, 10, 0, 15),
      "Decimal",
      "Alter"
    );
    const rightOperand: ArrayOperandNode = new ArrayOperandNode(
      [item],
      ["Alter"],
      IndexRange.create(0, 10, 0, 15),
      "Decimal",
  ...
  test("getCompletionContainer with incomplete OperationNode and name-filter, expected Operands", () => {
    ...
    const actual:
      | string[]
      | undefined
      | null = (operation.getCompletionContainer(positionParameter)
      .$transitions[0] as OperandTransition).$nameFilter;
    expect(actual).toEqual(expected);
  });
