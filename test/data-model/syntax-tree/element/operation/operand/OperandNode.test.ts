...
import {
    SyntaxHighlightingCapture
} from '../../../../../../src/provider/syntax-highlighting/SyntaxHighlightingCapture';
...
describe('OperandNode Tests', () => {
    ...
    test('getBeautifiedContent test, expect not empty content', () => {
        ...
        const actual = operandNode.getBeautifiedContent(initializer.$server.aliasHelper);
    ...
    test('getCompletionContainer test, expect empty transition list', () => {
        ...
        const actual: StateTransition[] = operandNode.getCompletionContainer(Position.create(0, 5)).$transitions;
    ...
    test('getPatternInformation with single operand, expect variable', () => {
        ...
        const actual: ScopeEnum[] = operandNode.getPatternInformation(initializer.$server.aliasHelper)!.$capture;
    ...
    test('getPatternInformation with complex static decimal operand, expect empty, static number and empty', () => {
        ...
        const actual: ScopeEnum[] = operandNode.getPatternInformation(initializer.$server.aliasHelper)!.$capture;
        const expected: ScopeEnum[] = [ScopeEnum.Empty, ScopeEnum.StaticNumber, ScopeEnum.Empty];
    ...
    test('getPatternInformation with complex schema operand, expect empty, variable', () => {
        ...
        const actual: ScopeEnum[] = operandNode.getPatternInformation(initializer.$server.aliasHelper)!.$capture;
        const expected: ScopeEnum[] = [ScopeEnum.Empty, ScopeEnum.Variable, ScopeEnum.Keyword, ScopeEnum.Variable];
    ...
    test('getPatternInformation with empty lines, expect null', () => {
        ...
        const actual: SyntaxHighlightingCapture | null =
            operandNode.getPatternInformation(initializer.$server.aliasHelper);
