...
describe('FunctionOperandNode Tests', () => {
    ...
    test('FunctionOperandNode get type and acceptedType test', () => {
        ...
        const functionNode: FunctionOperandNode =
            new FunctionOperandNode(
                [], [functionString], IndexRange.create(0, 0, 0, functionString.length), 'Decimal', operand, 'Decimal'
            );
    ...
    test('getChildren with one child, expect one child', () => {
        ...
        const functionString: string = 'SUM OF ' + operand;
        const functionNode: FunctionOperandNode =
            new FunctionOperandNode([operandNode], [functionString], IndexRange.create(0, 0, 0, functionString.length), 'Decimal', operand, 'Decimal');
    ...
    test('getChildren with previous set children, expect one child', () => {
        ...
        const operandNode: OperandNode = new OperandNode([operand], IndexRange.create(0, 0, 0, operand.length), 'Decimal', operand);
    ...
    test('getBeautifiedContent test, expect not empty content', () => {
        ...
        const functionNode: FunctionOperandNode =
            new FunctionOperandNode(
                [operandNode], [functionString], IndexRange.create(0, 0, 0, functionString.length), 'Decimal', operand, 'Decimal'
            );

        const actual = functionNode.getBeautifiedContent(initializer.$server.aliasHelper);
    ...
    test('getCompletionContainer without children, expect empty operand transition list', () => {
        ...
        const actual: StateTransition[] = functionNode.getCompletionContainer(Position.create(0, 5)).$transitions;
    ...
    test('getPatternInformation with single operand, expect variable', () => {
        ...
        const actual: ScopeEnum[] = functionNode.getPatternInformation(initializer.$server.aliasHelper)!.$capture;
    ...
    test('getPatternInformation with single static decimal operand, expect static number', () => {
        ...
        const operandNode: OperandNode = new OperandNode([operand], IndexRange.create(0, 0, 0, operand.length), 'Decimal', operand, true);
        const functionString: string = 'SUM OF ' + operand;
        const functionNode: FunctionOperandNode =
            new FunctionOperandNode([operandNode], [functionString], IndexRange.create(0, 0, 0, functionString.length), 'Decimal', operand, 'Decimal');
    ...
    test('getPatternInformation with complex static decimal operand, expect empty and static number', () => {
        ...
        const operandNode: OperandNode = new OperandNode([operand], IndexRange.create(0, 0, 0, operand.length), 'Decimal', '100', true);
        const functionString: string = 'SUM OF ' + operand;
        const functionNode: FunctionOperandNode =
            new FunctionOperandNode([operandNode], [functionString], IndexRange.create(0, 0, 0, functionString.length), 'Decimal', operand, 'Decimal');

        const actual: ScopeEnum[] = functionNode.getPatternInformation(initializer.$server.aliasHelper)!.$capture;
        const expected: ScopeEnum[] = [ScopeEnum.Keyword, ScopeEnum.Empty, ScopeEnum.StaticNumber];
    ...
    test('getPatternInformation with complex static decimal operand, expect empty, static number and empty', () => {
        ...
        const actual: ScopeEnum[] = functionNode.getPatternInformation(initializer.$server.aliasHelper)!.$capture;
        const expected: ScopeEnum[] = [ScopeEnum.Keyword, ScopeEnum.Empty, ScopeEnum.StaticNumber, ScopeEnum.Empty];
    ...
    test('getPatternInformation with complex schema operand, expect empty, variable', () => {
        ...
        const operandNode: OperandNode = new OperandNode([operand], IndexRange.create(0, 0, 0, operand.length), 'Decimal', 'Student.Alter');
        const functionString: string = 'SUM OF ' + operand;
        const functionNode: FunctionOperandNode =
            new FunctionOperandNode([operandNode], [functionString], IndexRange.create(0, 0, 0, functionString.length), 'Decimal', operand, 'Decimal');

        const actual: ScopeEnum[] = functionNode.getPatternInformation(initializer.$server.aliasHelper)!.$capture;
        const expected: ScopeEnum[] = [ScopeEnum.Keyword, ScopeEnum.Empty, ScopeEnum.Variable];
    ...
    test('getPatternInformation with complex schema operand, expect empty, variable', () => {
        ...
        const expected: ScopeEnum[] =
            [ScopeEnum.Keyword, ScopeEnum.Empty, ScopeEnum.Variable, ScopeEnum.Keyword, ScopeEnum.Variable];
