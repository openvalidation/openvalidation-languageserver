...
describe('OperatorNode Tests', () => {
    ...
    test('OperatorNode get dataType and validType test', () => {
        ...
        const operatorNode: OperatorNode = new OperatorNode([operator], IndexRange.create(0, 0, 0, operator.length), 'Boolean', operator, 'Decimal');
    ...
    test('getBeautifiedContent test, expect not empty content', () => {
        ...
        const actual = operatorNode.getBeautifiedContent(initializer.$server.aliasHelper);
    ...
    test('getCompletionContainer test, expect empty transition list', () => {
        ...
        const actual: StateTransition[] = operatorNode.getCompletionContainer(Position.create(0, 5)).$transitions;
    ...
    test('getPatternInformation with empty lines, expect null', () => {
        ...
        const actual: SyntaxHighlightingCapture | null =
            operatorNode.getPatternInformation(initializer.$server.aliasHelper);
    ...
    test('getPatternInformation with empty lines, expect keyword', () => {
        ...
        const actual: ScopeEnum[] = operatorNode.getPatternInformation(initializer.$server.aliasHelper)!.$capture;
