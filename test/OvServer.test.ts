...
describe('Dummy Tests', () => {
    ...
    test('initialize with rootPath in params, expect no error', async () => {
        expect(async () =>
            await initializer.$server['initialize'](
                { rootPath: 'test', rootUri: 'test', processId: 20, capabilities: {}, workspaceFolders: null })
        ).not.toThrow(Error);
    });
    ...
    test('initialize without rootPath but with rootUri in params, expect no error', async () => {
        expect(async () =>
            await initializer.$server['initialize'](
                { rootUri: 'test', processId: 20, capabilities: {}, workspaceFolders: null })).not.toThrow(Error);
    });

    test('initialize without paths in params, expect no error', async () => {
        expect(async () =>
            await initializer.$server['initialize'](
                { rootUri: null, processId: 20, capabilities: {}, workspaceFolders: null })).not.toThrow(Error);
    });

    test('validateAndSetSchemaDefinition with default params, expect no error', () => {
        expect(() =>
            initializer.$server['validateAndSetSchemaDefinition'](
                { schema: 'Alter: 20', uri: 'test.ov' })).not.toThrow(Error);
    });

    test('setLanguage with default params, expect no error', () => {
        expect(async () =>
            await initializer.$server.setLanguage({ language: 'Java', uri: 'test.ov' })).not.toThrow(Error);
    });
    ...
    test('setGeneratedSchema, expect no error', () => {
        expect(() =>
            initializer.$server.setGeneratedSchema(initializer.mockNotEmptyLintingResponse())).not.toThrow(Error);
    });
