...
describe('TreeTraversal Tests', () => {
    ...
    test('traverseTree with operation, expect matching child of operation', () => {
        ...
        const left: OperandNode =
            new OperandNode([operand], IndexRange.create(0, 0, 0, 4), 'String', operand);
        const operator: OperatorNode =
            new OperatorNode(
                ['kleiner'], IndexRange.create(0, 5, 0, 12), 'Boolean', 'kleiner', 'Decimal'
            );
        ...
        const operationNode: OperationNode =
            new OperationNode(left, operator, right, ['Test kleiner Test'], IndexRange.create(0, 0, 0, 17));
    ...
    test('getOperations with one variable with one operand and operation, expect empty list', () => {
        ...
        const operationNode: OperationNode =
            new OperationNode(left, operator, right, ['Test kleiner Test'], IndexRange.create(0, 0, 0, 'Test kleiner Test'.length));
        const variable: VariableNode = new VariableNode(null, operationNode, [], IndexRange.create(0, 0, 0, 0));
    ...
    test('getOperations with one empty variable, expect empty list', () => {
        const variable: VariableNode = new VariableNode(null, null, [], IndexRange.create(0, 0, 0, 0));
    ...
    test('getLonelyOperands with one variable with one operand and operation, expect empty list', () => {
        const operandText: string = 'Test';
        const operand: OperandNode = new OperandNode([operandText], IndexRange.create(0, 0, 0, operandText.length), 'String', operandText);
        const variable: VariableNode = new VariableNode(null, operand, [], IndexRange.create(0, 0, 0, 0));
